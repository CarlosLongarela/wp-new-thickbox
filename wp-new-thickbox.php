...
jQuery(function($) {
<?php
if ( 'js' === $this->options['wp_new_thickbox'] ) {
?>
...
if ( 'modified' === $this->options['thickbox_type'] ) {
	...
	if ( 'on' === $this->options['key_close_esc'] ) {
		$keys_close[] = 27;
	}

	if ( 'on' === $this->options['key_close_enter'] ) {
		$keys_close[] = 13;
	}
	...
	if ( 'on' === $this->options['key_prev_angle'] ) {
		$keys_prev[] = 188;
	}

	if ( 'on' === $this->options['key_prev_left'] ) {
		$keys_prev[] = 37;
	}

	if ( 'on' === $this->options['key_prev_tab'] ) {
		$keys_prev_shift[] = 9;
	}

	if ( 'on' === $this->options['key_prev_space'] ) {
		$keys_prev_shift[] = 32;
	}

	if ( 'on' === $this->options['key_prev_bs'] ) {
		$keys_prev[] = 8;
	}
	...
	if ( 'on' === $this->options['key_next_angle'] ) {
		$keys_next[] = 190;
	}

	if ( 'on' === $this->options['key_next_right'] ) {
		$keys_next[] = 39;
	}

	if ( 'on' === $this->options['key_next_tab'] ) {
		$keys_next[] = 9;
	}

	if ( 'on' === $this->options['key_next_space'] ) {
		$keys_next[] = 32;
	}
	...
	if ( 'on' === $this->options['key_end_home_end'] ) {
		$keys_first[] = 36;
		$keys_last[] = 35;
	}
	...
	if ( $script ) {
		echo "// Options for WP New ThickBox\n{$script}";
	}
}
?>
/* ]]> */
...
<?php
	...
	function custom_styles() {
		...
		if ( 'none' === $this->options['img_close_btn'] ) {
			$style .= "#TB_closeWindow { display: none; }\n";
		}

		if ( 'none' === $this->options['img_load'] ) {
			$style .= "#TB_load { display: none !important; }\n";
		} elseif ( ! $this->is_default_options( 'img_load' ) ) {
	...
	} // End of custom_styles().

	/**
	 * Check if is default option.
	 *
	 * @param string $names String name to check.
	 *
	 * @return boolean
	 **/
	...
	function add_wp_new_thickbox_action_links( $links, $file ) {
		...
		return $links;
	} // End of add_wp_new_thickbox_action_links( $links, $file ).

	/**
	 * Add additional links on the Plugins page.
	 *
	 * @param array  $links Array of links to add.
	 * @param string $file  File path.
	 *
	...
	function add_wp_new_thickbox_links( $links, $file ) {
		if ( plugin_basename( __FILE__ ) === $file ) {
			$links[] = '<a href="plugin-install.php?tab=plugin-information&plugin=wp-new-thickbox&TB_iframe" class="thickbox" title="Auto ThickBox Plus">' . esc_html__( 'Show Details', 'wp-new-thickbox' ) . '</a>';
			$links[] = '<a href="http://wordpress.org/support/plugin/wp-new-thickbox" target="_blank">' . esc_html__( 'Support', 'wp-new-thickbox' ) . '</a>';
			$links[] = '<a href="' . esc_html__('http://attosoft.info/en/') . 'contact/" target="_blank">' . esc_html__( 'Contact', 'wp-new-thickbox' ) . '</a>';
			$links[] = '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=7WT49YJDWJUCY" target="_blank">' . esc_html__( 'Donate', 'wp-new-thickbox' ) . '</a>';
		}
		return $links;
	}
